...
3. **Keep blocking I/O on explicit boundaries** - use `spawn_blocking` for known sync subsystems (DB, Redis, Spotify sync calls)
...
- Verify blocking I/O work uses explicit `tokio::task::spawn_blocking` boundaries
