...
mod tests {
    ...
    fn llm_error_display_all_variants() {
        let err = LlmError::Serialization("json error".to_string());
        assert_eq!(err.to_string(), "request serialization failed: json error");

        let err = LlmError::Request("connection failed".to_string());
        assert_eq!(err.to_string(), "HTTP request failed: connection failed");

        let err = LlmError::ResponseBody("read error".to_string());
        assert_eq!(err.to_string(), "failed to read response body: read error");
        ...
        assert_eq!(
            err.to_string(),
            "response deserialization failed: parse error"
        );
    }
    ...
    fn llm_error_implements_error_trait() {
        let err: Box<dyn std::error::Error> =
            Box::new(LlmError::MissingEnvVar("TEST".to_string()));
    ...
    fn from_env_fails_without_api_key() {
        ...
        if let Err(LlmError::MissingEnvVar(var)) = result {
    ...
    fn from_env_uses_defaults_when_optional_vars_missing() {
        // Set required var, clear optional vars
        ...
        assert!(client.config.system_prompt.is_none());
        assert!(client.config.temperature.is_none());

        env::remove_var(GEMINI_API_KEY);
    }

    #[test]
    fn from_env_respects_optional_env_vars() {
        env::set_var(GEMINI_API_KEY, "test-key");
        env::set_var(LLM_BASE_URL, "https://custom.api.com/v1");
        env::set_var(LLM_MODEL, "custom-model");

        let client = GeminiClient::from_env().unwrap();
        assert_eq!(client.config.api_key, "test-key");
        assert_eq!(client.config.base_url, "https://custom.api.com/v1");
    ...
    #[test]
    fn from_env_with_system_prompt_fails_without_api_key() {
        env::remove_var(GEMINI_API_KEY);

        let result = GeminiClient::from_env_with_system_prompt("test prompt");
        assert!(result.is_err());
    }

    #[test]
    fn with_temperature_accepts_zero() {
        let client = GeminiClient::new(sample_config())
            .with_temperature(0.0)
            .unwrap();
        assert_eq!(client.config.temperature, Some(0.0));
    ...
    #[test]
    fn with_temperature_accepts_max() {
        let client = GeminiClient::new(sample_config())
            .with_temperature(2.0)
            .unwrap();
        assert_eq!(client.config.temperature, Some(2.0));
    ...
    #[test]
    fn with_temperature_accepts_mid_range() {
        let client = GeminiClient::new(sample_config())
    ...
    #[test]
    fn with_temperature_rejects_negative_infinity() {
        let result = GeminiClient::new(sample_config()).with_temperature(f32::NEG_INFINITY);
        assert!(result.is_err());
        assert!(matches!(result, Err(LlmError::InvalidTemperature(_))));
    }

    #[test]
    fn with_temperature_rejects_just_below_zero() {
        let result = GeminiClient::new(sample_config()).with_temperature(-0.01);
        assert!(result.is_err());
    }

    #[test]
    fn with_temperature_rejects_just_above_max() {
        let result = GeminiClient::new(sample_config()).with_temperature(2.01);
        assert!(result.is_err());
    }

    #[test]
    fn chat_message_with_different_roles() {
    ...
    #[test]
    fn chat_completion_response_with_null_finish_reason() {
        let json = r#"{
            "choices": [{
                "index": 0,
    ...
    #[test]
    fn chat_completion_response_usage_all_none() {
        let json = r#"{
            "choices": [{
                "index": 0,
                "message": {"role": "assistant", "content": "test"},
    ...
    #[test]
    fn extract_reply_takes_first_choice_only() {
        let response = ChatCompletionResponse {
            choices: vec![
                Choice {
                    index: 0,
        ...
        let reply = GeminiClient::extract_reply(response).unwrap();
        assert_eq!(reply, "First choice");
    }

    #[test]
    fn build_messages_capacity_optimization() {
        let mut config = sample_config();
        config.system_prompt = Some("system".to_string());
        let client = GeminiClient::new(config);

        let messages = client.build_messages("user message");
        // Verify we get both system and user messages
        assert_eq!(messages.len(), 2);
    }
    ...
    #[test]
    fn config_with_temperature_is_cloneable() {
        let mut config = sample_config();
    ...
    #[test]
    fn request_body_serializes_with_messages() {
        let body = ChatCompletionRequest {
            model: "test-model".to_string(),
            messages: vec![ChatMessage {
                role: ChatRole::User,
    ...
    #[test]
    fn request_body_with_temperature_serializes_correctly() {
        let body = ChatCompletionRequest {
            model: "test-model".to_string(),
            messages: vec![],
    ...
    #[test]
    fn invalid_temperature_error_contains_value() {
        let err = LlmError::InvalidTemperature(5.0);
    ...
    #[test]
    fn deserialization_error_preserves_body() {
        let err = LlmError::Deserialization {
            message: "error".to_string(),
            body: "response body".to_string(),
        };
        // The display impl doesn't include body, but the variant does store it
        match err {
            LlmError::Deserialization { body, .. } => assert_eq!(body, "response body"),
            _ => panic!("wrong variant"),
        }
    }

    #[test]
    fn gemini_client_has_default_timeout() {
        let client = GeminiClient::new(sample_config());
        // The timeout is set but we can't directly inspect it without making a request
        // This test just ensures the client builds successfully with timeout config
        assert_eq!(client.model(), "test-model");
    }
    ...
    #[test]
    fn from_env_with_empty_string_values() {
        // Even if env vars are set to empty strings, they should be used
        env::set_var(GEMINI_API_KEY, "");
        env::set_var(LLM_BASE_URL, "");
    ...
    #[test]
    fn build_messages_does_not_mutate_client() {
        let client = GeminiClient::new(sample_config());
        let _msgs1 = client.build_messages("first");
        let _msgs2 = client.build_messages("second");
        // If this compiles and runs, build_messages doesn't require &mut self
        assert_eq!(client.model(), "test-model");
    }

    #[test]
    fn response_with_unknown_fields_deserializes() {
        // APIs may add fields; serde should ignore them
        let json = r#"{
            "choices": [{
                "index": 0,
    ...
    #[test]
    fn chat_completion_request_debug_output() {
